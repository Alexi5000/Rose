...
class QdrantDiagnostics:
    ...
    def __init__(self):
        ...
        print("🔍 Initializing Qdrant diagnostics...")
            ...
            try:
                if hasattr(info, "config") and hasattr(info.config, "params"):
                    ...
                    if vectors_conf:
                        ...
                        print(
                            f"   Distance metric: {vectors_conf.distance if hasattr(vectors_conf, 'distance') else 'Unknown'}"
                        )
            except Exception:
                print("   Vector configuration: Unable to extract")
        ...
        try:
            ...
            while batch_num < max_batches:
                try:
                    ...
                    self.errors.append(
                        {"test": "scroll", "batch": batch_num + 1, "error": str(e)[:200], "type": "UnexpectedResponse"}
                    )

                    # Try to continue with next batch (skip corrupted segment)
                    print("      Attempting to skip corrupted segment...")
                    if offset is not None:
                        offset = (
                            str(int(offset) + batch_size)
                    ...
                    self.errors.append(
                        {"test": "scroll", "batch": batch_num + 1, "error": str(e), "type": type(e).__name__}
                    )
                    break
        ...
        try:
            ...
            for query in test_queries:
                try:
                    ...
                    print(f"      Error: {str(e)[:200]}")
                    self.errors.append(
                        {"test": "search", "query": query, "error": str(e)[:200], "type": "UnexpectedResponse"}
                    )
                    return False
