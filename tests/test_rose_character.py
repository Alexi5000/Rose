...
class TestRoseCharacter:
    ...
    async def test_ancient_wisdom_metaphors(self, mock_chat):
        ...
        mock_llm = MagicMock()
        mock_llm.ainvoke = AsyncMock(
            return_value=AIMessage(content="Like a river that carves through stone, grief shapes us slowly, patiently.")
        )
        mock_chat.return_value = mock_llm
...
class TestTherapeuticScenarios:
    ...
    async def test_spiritual_awareness_response(self, mock_chat):
        ...
        mock_llm = MagicMock()
        mock_llm.ainvoke = AsyncMock(
            return_value=AIMessage(content="The ancestors remind us that death is not an ending, but a transformation.")
        )
        mock_chat.return_value = mock_llm
